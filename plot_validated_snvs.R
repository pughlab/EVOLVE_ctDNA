### plot_snv_vafs.R ################################################################################
# Identify and plot VAFs (tumour content)
...
input.data <- read.delim(
	'2021-06-04_EVOLVE_ctDNA_ensemble_oncokb_annotated_filtered.tsv',
	stringsAsFactors = FALSE
	);

clinical <- read.delim('sample_info.txt');

## For now, only include ALL_UNIQUE
clinical <- droplevels(clinical[which(clinical$BAM == 'allUNIQUE'),]);

# collect list of all samples
all.samples <- sort(as.character(unique(clinical$Sample)));
...
# get known mutations (from WXS + manual confirmation)
known.muts <- read.delim('EVOLVE_confirmed_mutations.bed', header = FALSE);
colnames(known.muts) <- c('Chromosome','Start','End','Sample');

known.muts$Patient <- sapply(known.muts$Sample, function(i) { unlist(strsplit(as.character(i),'_'))[1] } );

### FORMAT DATA ####################################################################################
# because this is tightly targeted, fix this one case
...
# indicate key fields
keep.fields <- c('Tumor_Sample_Barcode','Hugo_Symbol','Chromosome','Start_Position','End_Position','Variant_Classification','Reference_Allele','Tumor_Seq_Allele2','t_depth','t_ref_count');
...
known.gr <- makeGRangesFromDataFrame(known.muts[,-4],
...
mutation.data.filtered <- as.data.frame(
	subsetByOverlaps(
		mutation.gr,
		subsetByOverlaps(known.gr, target.gr, ignore.strand = TRUE),
		ignore.strand = TRUE
		)
	);
...
create.dotmap(
	...
	resolution = 200,
	filename = generate.filename('EVOLVE_ctDNA_allUNIQUE', '_snv_landscape','png')
	);


tmp.vaf.data <- merge(
	clinical,
	aggregate(t_vaf ~ Tumor_Sample_Barcode + Hugo_Symbol, mutation.data.trimmed, max),
	by.x = 'Sample',
	by.y = 'Tumor_Sample_Barcode',
	all = TRUE
	);

tmp.vaf.data$Plot.group <- factor(1,levels = c(0,1));
tmp.vaf.data[which(tmp.vaf.data$Group == 'Screening'),]$Plot.group <- 0;

create.boxplot(
	t_vaf ~ Plot.group | Hugo_Symbol,
	tmp.vaf.data,
	add.stripplot = TRUE,
	points.alpha = 0.8,
	xlab.label = NULL,
	...
	ylab.axis.padding = 2,
	layout = c(6,1),
	xaxis.lab = c('baseline','on/end-of-trial'),
	xaxis.rot = 90,
	ylimits = c(0,1),
	yat = seq(0,1,0.2)
	);
