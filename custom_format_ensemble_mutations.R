...
# custom version for ctDNA with no matched normals
...
#	3) is called by MuTect2
#	4) keep position if it passes any of the above in any sample (for this patient)
...
if ('MuTect2' %in% colnames(combined.data)) {
	m2.idx <- which(combined.data$MuTect2 == 1);
	if (length(m2.idx) > 0) { combined.data[m2.idx,]$FILTER <- 'PASS'; }
	}
...
combined.data$Patient <- substr(combined.data$Tumor_Sample_Barcode, 0, 11);

passed.variants <- unique(combined.data[which(combined.data$FILTER == 'PASS'),c('Chromosome','Start_Position','End_Position','Reference_Allele','Tumor_Seq_Allele2','Variant_Type','Patient')]);
...
remove.idx <- which(is.na(keep.calls$t_depth) & is.na(keep.calls$n_depth));

if (length(remove.idx) > 0) { keep.calls <- keep.calls[-remove.idx,]; }
