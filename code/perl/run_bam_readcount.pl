...
sub main {
	...
	my $log_file = join('/', $log_directory, 'run_BAM-READCOUNT_pipeline.log');
	...
	unless ($args{dry_run}) {
		...
		$log_file = join('/', $log_directory, 'run_BAM-READCOUNT_pipeline_' . $run_count . '.log');
		}
	...
	print $log "---\n";
	print $log "Running BAM-READCOUNT pipeline.\n";
	...
	my $bam_readcount	= 'bam-readcount/0.7.4';
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		foreach my $sample (@samples) {
			...
			my $output_file = join('/', $patient_directory, $sample . '_readcounts.tsv');
			$cleanup_cmd .= "\nrm $output_file;";
			...
			if ('Y' eq missing_file($output_file)) {
				...
				$run_script = write_script(
					...
					name	=> 'run_bam_readcount_' . $sample,
					cmd	=> $readcount_command,
					modules	=> [$bam_readcount],
					max_time	=> '01:00:00',
					mem		=> '1G',
				...
				$run_id = submit_job(
					jobname		=> 'run_bam_readcount_' . $sample,
