...
sub get_readcount_command {
	...
	my $command = 'samtools view -b ' . join(' ',
		$args{input},
		'-L', $args{targets},
		'| bedtools coverage -a', $args{targets},
		'-b stdin -mean',
		'>', $args{output}
		);
...
sub main {
	...
	my $log_file = join('/', $log_directory, 'run_BEDTOOLS_COVERAGE_pipeline.log');
	...
	unless ($args{dry_run}) {
		...
		$log_file = join('/', $log_directory, 'run_BEDTOOLS_COVERAGE_pipeline_' . $run_count . '.log');
		}
	...
	print $log "---\n";
	print $log "Running BEDTOOLS COVERAGE pipeline.\n";
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		foreach my $sample (@samples) {
			...
			my $output_file = join('/', $patient_directory, $sample . '_interval_meanCoverage.tsv');
			$cleanup_cmd .= "\nrm $output_file;";
			...
			if ('Y' eq missing_file($output_file)) {
				...
				print $log "Submitting job for bedtools-coverage...\n";

				$run_script = write_script(
					log_dir	=> $log_directory,
					name	=> 'run_bedtools_coverage_' . $sample,
				...
				$run_id = submit_job(
					jobname		=> 'run_bedtools_coverage_' . $sample,
				...
				print $log "Skipping bedtools-coverage because this has already been completed!\n";
				}
