	...
	### Cox modelling analysis ###
		...
		if (ph.assumption.check != "ignore") {  
			if (length(levels(patient.groups)) == 1) {
				...
				ph.failed <- ph.fails(cox.model = cox.model, cox.zph.threshold = cox.zph.threshold, pvalues = FALSE);
				pvalues.zph <- ph.fails(cox.model = cox.model, cox.zph.threshold = cox.zph.threshold, pvalues = TRUE);
...
#					} 
	...
	if ('ttest' == statistical.method) { 
		...
		ret.stats <- ttest.analysis(values = survival.object[,1], groups = patient.groups, alternative = "two.sided");
	...
	if ('logrank' == statistical.method) { 
		...
		ret.stats <- logrank.analysis(survival.object, patient.groups);
